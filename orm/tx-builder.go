...
func (b *txQuery) Insert() error {
	...
	err := Insert(b.tx.Transaction.Ctx, b.tx, b.scope)
	if err != nil {
		b.tx.err = err
	}
	return err
...
func (b *txQuery) Update() (modelsAffected int64, err error) {
	...
	modelsAffected, err = Update(b.tx.Transaction.Ctx, b.tx, b.scope)
	if err != nil {
		b.tx.err = err
	}
...
// UpdateAll updates the scope's attribute and relationship values based on the scope models or filters.
func (b *txQuery) UpdateAll() (modelsAffected int64, err error) {
	if b.tx.err != nil {
		return 0, b.tx.err
	}

	modelsAffected, err = UpdateAll(b.tx.Transaction.Ctx, b.tx, b.scope)
	if err != nil {
		b.tx.err = err
	}
	return modelsAffected, err
...
func (b *txQuery) Find() ([]mapping.Model, error) {
	...
	values, err := Find(b.tx.Transaction.Ctx, b.tx, b.scope)
	if err != nil {
		b.tx.err = err
		return nil, err
	}
...
func (b *txQuery) Delete() (int64, error) {
	...
	modelsAffected, b.tx.err = Delete(b.tx.Transaction.Ctx, b.tx, b.scope)
	return modelsAffected, b.tx.err
}

// DeleteAll deletes all the models for the collection in the query's scope.
func (b *txQuery) DeleteAll() (int64, error) {
	if b.tx.err != nil {
		return 0, b.tx.err
	}
	var modelsAffected int64
	modelsAffected, b.tx.err = DeleteAll(b.tx.Transaction.Ctx, b.tx, b.scope)
	return modelsAffected, b.tx.err
}
...
func (b *txQuery) Filter(filterField *query.FilterField) Builder {
	if b.tx.err != nil {
		return b
	}
...
func (b *txQuery) OrderBy(fields ...*query.SortField) Builder {
	if b.tx.err != nil {
		return b
	}
...
func (b *txQuery) AddRelations(relationField *mapping.StructField, relations ...mapping.Model) error {
	...
	err := AddRelations(b.tx.Transaction.Ctx, b.tx, b.scope, relationField, relations...)
	if err != nil {
		b.tx.err = err
	}
	return err
...
func (b *txQuery) SetRelations(relationField *mapping.StructField, relations ...mapping.Model) error {
	...
	err := SetRelations(b.tx.Transaction.Ctx, b.tx, b.scope, relationField, relations...)
	if err != nil {
		b.tx.err = err
	}
	return err
...
func (b *txQuery) RemoveRelations(relationField *mapping.StructField) (int64, error) {
	...
	modelsAffected, err := RemoveRelations(b.tx.Transaction.Ctx, b.tx, b.scope, relationField)
	if err != nil {
		b.tx.err = err
	}
	return modelsAffected, err
