...
func Insert(ctx context.Context, db DB, s *query.Scope) (err error) {
	...
	if len(s.FieldSet) == 0 {
		switch len(s.Models) {
		case 1:
			s.FieldSet, err = createInsertFieldSet(0, startTS, s)
			if err != nil {
				return err
			}
		default:
			err = createBulkInsertFieldSet(s, startTS)
			if err != nil {
				return err
			}
		}
...
func createInsertFieldSet(i int, startTS time.Time, s *query.Scope) (mapping.FieldSet, error) {
	model := s.Models[i]
...
func createBulkInsertFieldSet(s *query.Scope, startTS time.Time) error {
	s.BulkFieldSets = &query.BulkFieldSet{
		Indices: map[string][]int{},
	}
	for i := range s.Models {
		fieldSet, err := createInsertFieldSet(i, startTS, s)
		if err != nil {
			return err
		}
