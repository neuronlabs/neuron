...
func Find(ctx context.Context, db DB, s *query.Scope) ([]mapping.Model, error) {
	// If no fields were selected - the query searches for all fields.
	if len(s.FieldSet) == 0 {
		s.FieldSet = s.ModelStruct.Fields()
	}
...
func Get(ctx context.Context, db DB, s *query.Scope) (mapping.Model, error) {
	var err error
	// Check if the pagination is already set.
	if s.Pagination != nil {
		return nil, errors.Newf(query.ClassInvalidField, "cannot get single model with custom pagination")
	}
	// Assure that the result would be only a single value.
	s.Limit(1)
	results, err := Find(ctx, db, s)
	if err != nil {
		return nil, err
	}
	// if there is no result return an error of class QueryValueNoResult.
	if len(results) == 0 {
		return nil, errors.New(query.ClassNoResult, "values not found")
	}
...
func filterSoftDeleted(s *query.Scope) {
	...
	s.Filters = append(s.Filters, query.NewFilterField(deletedAt, query.OpIsNull))
}
...
func findBelongsToRelation(ctx context.Context, db DB, s *query.Scope, included *query.IncludedRelation) error {
	...
	relatedModels, err := db.QueryCtx(ctx, relationship.Struct()).
		Filter(query.NewFilterField(relationship.Struct().Primary(), query.OpIn, filterValues...)).
	...
	for _, relModel := range relatedModels {
		...
		for _, index := range indexes {
			...
			if !ok {
				return errors.Newf(mapping.ClassModelNotImplements, "model: '%s' doesn't implement mapping.SingleRelationer", s.ModelStruct.String())
			}
...
func findHasRelation(ctx context.Context, db DB, s *query.Scope, included *query.IncludedRelation) error {
	...
	relatedModels, err := db.QueryCtx(ctx, relationship.Struct()).
		Filter(query.NewFilterField(relationship.ForeignKey(), query.OpIn, filterValues...)).
		Select(fieldSet...).
		Find()
	if err != nil {
		return err
	...
	for _, relatedModel := range relatedModels {
		...
		case mapping.RelHasOne:
			relationer, ok := s.Models[index].(mapping.SingleRelationer)
			if !ok {
				return errors.Newf(mapping.ClassModelNotImplements, "model: '%s' does not implement mapping.SingleRelationer interface", s.ModelStruct.String())
			}
		...
		case mapping.RelHasMany:
			...
			if !ok {
				return errors.Newf(mapping.ClassModelNotImplements, "model: '%s' does not implement mapping.MultiRelationer interface", s.ModelStruct.String())
			}
...
func findManyToManyRelation(ctx context.Context, db DB, s *query.Scope, included *query.IncludedRelation) error {
	...
	models, err := db.QueryCtx(ctx, relationship.JoinModel()).
		...
		Filter(query.NewFilterField(backReferenceFK, query.OpIn, filterValues...)).
		Find()
	if err != nil {
		return err
	}
	...
	relationModels, err := db.QueryCtx(ctx, relationship.Struct()).
		Filter(query.NewFilterField(relationship.Struct().Primary(), query.OpIn, relationPrimaries...)).
		Select(fieldSet...).
		Find()
	if err != nil {
		return err
	...
	for _, model := range relationModels {
		...
		for _, index := range indexes {
			relationer, ok := s.Models[index].(mapping.MultiRelationer)
			if !ok {
				return errors.Newf(mapping.ClassModelNotImplements, "model: '%s' doesn't implement interface MultiRelationer", s.ModelStruct)
			}
...
func findManyToManyRelationShort(s *query.Scope, included *query.IncludedRelation, models []mapping.Model, primariesToIndex map[interface{}]int) error {
	...
	for _, model := range models {
		...
		if !ok {
			return errors.Newf(mapping.ClassModelNotImplements, "model: '%s' doesn't implement MultiRelationer", s.ModelStruct.String())
		}
...
func errModelNotAFielder(s *query.Scope) errors.ClassError {
