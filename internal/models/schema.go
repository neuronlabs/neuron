...
// // NewModelModelss create new ModelModelss based on the config
// func NewModelModelss(
// 	namerFunc namer.Namer,
// 	c *config.Controller,
// ) (*ModelModelss, error) {
// 	return newModelModelss(namerFunc,
// 		c.ModelModelss,
// 		c.DefaultModels,
...
// func newModelModelss(
// 	namerFunc namer.Namer,
// 	cfg map[string]*config.Models,
// 	defaultModels string,
// 	defaultRepoName string,
// ) (*ModelModelss, error) {
// 	log.Debugf("Creating New ModelModelss...")
// 	ms := &ModelModelss{
// 		NamerFunc:       namerFunc,
// 		cfg:             cfg,
...
// 	ms.defaultModels = &Models{Name: defaultModels, models: NewModelMap()}
// 	ms.schemas = make(map[string]*Models)
// 	ms.schemas[defaultModels] = ms.defaultModels

// 	// set the schema's configs
// 	for name, schemaCfg := range cfg {
// 		if name == defaultModels {
// 			ms.defaultModels.config = schemaCfg
// 		} else {
// 			ms.schemas[name] = &Models{
// 				config: schemaCfg,
// 				Name:   name,
// 				models: NewModelMap(),
// 			}
// 		}
// 		log.Debugf("Models %s created with config", name)
// 	}
...
// func (m *ModelModelss) ComputeNestedIncludedCount(limit int) {
...
// // DefaultModels returns default schema for give models
// func (m *ModelModelss) DefaultModels() *Models {
// 	return m.defaultModels
// }

// // Models returns schema on the base of schemaName
// func (m *ModelModelss) Models(schemaName string) (*Models, bool) {
// 	s, ok := m.schemas[schemaName]
// 	return s, ok
// }

// // Modelss returns all registered schemas
// func (m *ModelModelss) Modelss() []*Models {
// 	var schemas []*Models
...
// func (m *ModelModelss) RegisterModels(models ...interface{}) error {
// 	// iterate over models and register one by one
// 	for _, model := range models {
// 		var schema string

// 		// set model's schema
// 		if schemaNamer, ok := model.(ModelsNamer); ok {
// 			schema = schemaNamer.ModelsName()
// 		} else {
// 			schema = m.defaultModels.Name
// 		}

// 		// check if schema is already created
// 		s, ok := m.schemas[schema]
// 		if !ok {
// 			log.Debugf("Models: %s not found for the model. Creating new schema.", schema)
// 			s = &Models{
// 				Name:   schema,
// 				models: NewModelMap(),
// 			}
...
// // RegisterModelsModels registers models for provided schema
// func (m *ModelModelss) RegisterModelsModels(schemaName string, models ...interface{}) error {
// 	/**

// 	TO DO:

// 	- Get Models from name
...
// func (m *ModelModelss) RegisterModelsRecursively(models ...interface{}) error {
// 	/**
...
// func (m *ModelModelss) setModelRelationships(model *ModelStruct) error {
// 	schema := m.schemas[model.ModelsName()]
...
// func (m *ModelModelss) setRelationships() error {
// 	for _, schema := range m.schemas {
...
// func (m *ModelModelss) GetModelStruct(model interface{}) (*ModelStruct, error) {
// 	return m.getModelStruct(model)
// }

// // ModelByType returns the model on the base of the provided model type
// func (m *ModelModelss) ModelByType(t reflect.Type) (*ModelStruct, error) {
// 	return m.getByType(t)
// }

// // ModelsByType returns schema by the type provided in the arguments
// func (m *ModelModelss) ModelsByType(t reflect.Type) (*Models, error) {
// 	return m.getModelsByType(t)
// }

// func (m *ModelModelss) getByType(t reflect.Type) (*ModelStruct, error) {
...
// 	schemaNamer, ok := reflect.New(t).Interface().(ModelsNamer)
// 	if ok {
// 		schemaName = schemaNamer.ModelsName()
// 		log.Debugf("Models Namer: %T", schemaNamer)
// 	} else {
// 		schemaName = m.defaultModels.Name
// 	}

// 	schema, ok := m.schemas[schemaName]
// 	if !ok {
// 		return nil, errors.Newf(class.ModelModelsNotFound, "schema: '%s' not found", schemaName)
// 	}

// 	mStruct := schema.models.Get(t)
// 	if mStruct == nil {
// 		return nil, errors.Newf(class.ModelNotMappedInModels, "model: '%s' is not found within schema: '%s'", t.Name(), schemaName)
// 	}
...
// func (m *ModelModelss) getModelsByType(t reflect.Type) (*Models, error) {
// 	if t.Kind() == reflect.Ptr {
// 		t = t.Elem()
// 	}
// 	if t.Kind() == reflect.Slice {
...
// 	schema, ok := m.schemas[schemaName]
// 	if !ok {
// 		return nil, errors.Newf(class.ModelModelsNotFound, "model schema: '%s' not found", schemaName)
// 	}
// 	return schema, nil
// }
