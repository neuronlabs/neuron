...
import (
	...
	"github.com/kucjac/jsonapi/pkg/internal/gateway/endpoints"
...
var (
	...
	IErrModelNotFound = errors.New("Model Handler not found.")
)
...
type Handler struct {
	...
	// endpoints.Models
	modelHandlers map[models.ModelStruct]*endpoints.Model
}
...
func NewHandler(
	...
	h := &Handler{
		Controller:      c,
		modelHandlers:   make(map[reflect.Type]*endpoints.Model),
...
// // Addendpoints.Models adds the model handlers for given JSONAPI Handler.
// // If there are handlers with the same type the funciton returns error.
// func (h *Handler) Addendpoints.Models(models ...*endpoints.Model) error {
// 	for _, model := range models {
// 		if _, ok := h.endpoints.Models[model.ModelType]; ok {
// 			err := fmt.Errorf("endpoints.Model of type: '%s' is already inside the Handler", model.ModelType.Name())
// 			return err
// 		}
// 		h.endpoints.Models[model.ModelType] = model
// 	}
// 	return nil
// }
...
func (h *Handler) GetRelationshipFilters(
	...
	for i, relFilter := range scope.RelationshipFilters {
		...
		relModel, ok := h.endpoints.Models[relFilter.GetRelatedModelType()]
		if !ok {
...
func (h *Handler) getModelRepositoryByType(modelType reflect.Type) (repo Repository) {
	model, ok := h.endpoints.Models[modelType]
	if !ok {
