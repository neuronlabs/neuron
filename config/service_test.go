...
func TestParseRepository(t *testing.T) {
	type testcase struct {
		name string
		s    string
		tf   func(t *testing.T, r *Service, err error)
	}
	...
	tests := []testcase{
		{
			...
			func(t *testing.T, r *Service, err error) {
				require.NoError(t, err)

				assert.Equal(t, "172.16.1.1", r.Connection.Host)
				assert.Equal(t, 5432, r.Connection.Port)
				assert.Equal(t, "some", r.Connection.Username)
				assert.Equal(t, "pass", r.Connection.Password)
				assert.Equal(t, "postgres", r.DriverName)
				assert.Equal(t, "some_db", r.DBName)
				assert.Equal(t, time.Minute*5, *r.Connection.MaxTimeout)
				assert.Equal(t, "some://raw.url", r.Connection.RawURL)
			},
		},
		{
			"InvalidEqualSymbol",
