...
func reduceRelationshipFilter(ctx context.Context, db DB, s *query.Scope, relationFilter filter.Relation) error {
	...
	default:
		return errors.Newf(filter.ClassFilterField, "filter's field: '%s' is not a relationship", relationFilter.StructField)
	}
}
...
func reduceBelongsToRelationshipFilter(ctx context.Context, db DB, s *query.Scope, f filter.Relation) error {
	...
	relatedQuery := db.QueryCtx(ctx, f.StructField.Relationship().Struct())
	for _, nestedFilter := range f.Nested {
		relatedQuery.Filter(nestedFilter)
	}

	models, err := relatedQuery.Select(f.StructField.Relationship().Struct().Primary()).Find()
	if err != nil {
		return err
	}
	if len(models) == 0 {
		log.Debug2f(logFormat(s, "no belongs to relationship: '%s' results found"), f.StructField)
		return errors.Newf(query.ClassNoResult, "no relationship: '%s' filter results found", f.StructField)
	}
...
func reduceHasManyRelationshipFilter(ctx context.Context, db DB, s *query.Scope, relationFilter filter.Relation) error {
	...
	relationQuery := db.QueryCtx(ctx, relationFilter.StructField.Relationship().Struct())
	for _, nested := range relationFilter.Nested {
	...
	for _, relationModel := range relationModels {
		...
		if !ok {
			return errors.Newf(mapping.ClassModelNotImplements, "model: '%s' doesn't implement Fielder", relationFilter.StructField.Relationship().Struct())
		}
	...
	if len(uniqueForeignKeyValues) == 0 {
		...
		return errors.Newf(query.ClassNoResult, "no relationship: '%s' filter filterValues found", relationFilter.StructField)
	}
...
func reduceMany2ManyRelationshipFilter(ctx context.Context, db DB, s *query.Scope, relationFilter filter.Relation) error {
	...
	onlyPrimaries := true
	relatedPrimaryKey := relationFilter.StructField.Relationship().Struct().Primary()
	foreignKey := relationFilter.StructField.Relationship().ForeignKey()
	...
	relatedModels, err := relationQuery.Select(relationFilter.StructField.Relationship().Struct().Primary()).Find()
	if err != nil {
		return err
	}
	if len(relatedModels) == 0 {
		log.Debug2f(logFormat(s, "No relationship: '%s' filter results found"), relationFilter.StructField)
		return errors.Newf(query.ClassNoResult, "no relationship: '%s' filter results", relationFilter.StructField)
	}
