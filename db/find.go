...
// Find gets the values from the repository with respect to the query filters, sorts, pagination and included values.
...
func Find(ctx context.Context, db DB, s *query.Scope) ([]mapping.Model, error) {
	...
	default:
		return nil, errors.NewDetf(query.ClassInvalidFieldSet, "provided too many fieldsets for the find query")
	}
...
func Get(ctx context.Context, db DB, s *query.Scope) (mapping.Model, error) {
	...
	if len(results) == 0 {
		return nil, errors.New(query.ClassNoResult, "model not found")
	}
...
func findBelongsToRelation(ctx context.Context, db DB, s *query.Scope, included *query.IncludedRelation) error {
	...
	relatedModels, err := db.QueryCtx(ctx, relationship.Struct()).
		Filter(filter.New(relationship.Struct().Primary(), filter.OpIn, filterValues...)).
...
func findManyToManyRelation(ctx context.Context, db DB, s *query.Scope, included *query.IncludedRelation) error {
	...
	relationModels, err := db.QueryCtx(ctx, relationship.Struct()).
		Filter(filter.New(relationship.Struct().Primary(), filter.OpIn, relationPrimaries...)).
		Select(fieldSet...).
		Find()
	if err != nil {
		return err
