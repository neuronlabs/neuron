...
func (c *Controller) healthCheckRepo(ctx context.Context, repo repository.HealthChecker, wg *sync.WaitGroup, errChan chan<- error, hc chan<- *repository.HealthResponse) {
	go func() {
		defer wg.Done()
