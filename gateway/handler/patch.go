...
func (h *Handler) HandlePatch(m *mapping.ModelStruct) http.HandlerFunc {
	return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		id, err := builder.GetAndSetID(req, (*iscope.Scope)(s))
		if err != nil {
			...
			default:
				...
				errObj.Detail = "Provided invalid 'id' in the builder."
			}
		...
		if errs := (*query.Scope)(s).ValidatePatch(); len(errs) > 0 {
			h.marshalErrors(req, rw, unsetStatus, errs...)
			return
		}

		// Patch the values
		if err := (*query.Scope)(s).Patch(); err != nil {
			log.Debugf("Patching the scope: '%s' failed. %v", (*query.Scope)(s).ID().String(), err)
		...
		if ok && v {
			...
			if err := (*query.Scope)(s).Get(); err != nil {
				log.Debugf("Getting the Patched scope: '%s' failed. %v", (*query.Scope)(s).ID().String(), err)
				h.handleDBError(req, err, rw)
				return
			}
